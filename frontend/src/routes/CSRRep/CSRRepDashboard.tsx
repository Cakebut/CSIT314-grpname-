            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24, justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  let resp;
                  try {
                    if (shortlistedIds.includes(selected.requestId)) {
                      resp = await fetch(`http://localhost:3000/api/csr/${csrId}/shortlist/${selected.requestId}`, { method: "DELETE" });
                    } else {
                      resp = await fetch(`http://localhost:3000/api/csr/${csrId}/shortlist/${selected.requestId}`, { method: "POST" });
                    }
                    if (!resp.ok) {
                      const err = await resp.text();
                ...
                aria-label={shortlistedIds.includes(selected.requestId) ? "Remove from shortlist" : "Add to shortlist"}
                title={shortlistedIds.includes(selected.requestId) ? "Remove from shortlist" : "Add to shortlist"}
                onMouseDown={e => e.currentTarget.style.transform = 'scale(0.92)'}
                onMouseUp={e => e.currentTarget.style.transform = 'scale(1)'}
                onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}
              >
                <span style={{
                  color: shortlistedIds.includes(selected.requestId) ? '#ef4444' : '#64748b',
                  fontSize: 28,
                  lineHeight: 1,
                  transition: 'color 0.15s',
                  userSelect: 'none',
                }}>
                  {shortlistedIds.includes(selected.requestId) ? '❤️' : '🤍'}
                </span>
              </button>
            </div>
          </div>
