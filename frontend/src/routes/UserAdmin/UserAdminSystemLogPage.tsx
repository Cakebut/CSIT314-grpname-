...
export default function UserAdminSystemLogPage() {
  ...
  useEffect(() => {
    ...
    const fetchAndUpdate = () => {
      fetchAuditLogs(limit)
        ...
        .catch(() => { if (isMounted) setError("Failed to load logs"); })
  ...
  return (
    <div className="system-log-container modern-log">
      ...
      <div className="log-header">
        ...
        <div className="log-controls" style={{ flexWrap: 'wrap', gap: '1.2rem', background: '#f3f6fb', borderRadius: '12px', boxShadow: '0 2px 8px rgba(44,62,80,0.06)', padding: '1rem 1.2rem', marginTop: '0.5rem', display: 'flex', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem' }}>
            <label htmlFor="log-limit" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontWeight: 500 }}>
              <FaFilter style={{ marginRight: 2, color: '#64748b' }} /> Show
              <select id="log-limit" value={limit} onChange={e => setLimit(Number(e.target.value) || undefined)} style={{ marginLeft: 4 }}>
            ...
            <label htmlFor="action-filter" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontWeight: 500 }}>
              <FaFilter style={{ marginRight: 2, color: '#64748b' }} /> Action
              <select id="action-filter" value={actionFilter} onChange={e => setActionFilter(e.target.value)} style={{ marginLeft: 4 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem' }}>
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              <FaSearch style={{ position: 'absolute', left: 10, color: '#64748b', fontSize: '1.1em' }} />
              <input
                type="text"
                placeholder="Search logs..."
                value={actionSearch}
                onChange={e => setActionSearch(e.target.value)}
                style={{ padding: '0.4em 1em 0.4em 2.2em', borderRadius: 8, border: '1.2px solid #bfc8d6', fontSize: '1em', minWidth: 180 }}
              />
            </div>
            ...
            <tbody>
              {(() => {
                ...
                if (filtered.length === 0) {
                  return <tr><td colSpan={5} className="log-muted">No log entries found.</td></tr>;
                }
                return filtered.map(log => (
                  <tr key={log.id}>
                    <td><span className="log-user">{log.actor}</span></td>
                    <td><span className={`log-action log-action-${log.action.replace(/\s/g, "-")}`}>{log.action}</span></td>
                    <td><span className="log-target">{log.target}</span></td>
                    <td><span className="log-timestamp">{new Date(log.timestamp).toLocaleString()}</span></td>
